...
function ClubHubPostCard({ post }: { post: FeedCard }) {
  ...
  return (
    <article className="flex gap-3 rounded border border-primary/12 bg-white p-4  sm:p-[18px]">
      <Link
        aria-label={`View ${post.author}'s profile`}
        className="flex h-10 w-10 shrink-0 items-center justify-center rounded border border-primary/15 bg-primary-fixed text-xs font-bold text-black"
      ...
      <div className="min-w-0 flex-1">
        ...
        {mediaUrls.length > 0 ? (
          <div className={cn("mt-3 grid overflow-hidden rounded border border-[#deded8]", mediaUrls.length > 1 && "grid-cols-2 gap-px bg-[#deded8]")}>
        ...
        {tag ? (
          <span className="mt-3 inline-flex rounded border border-secondary/15 bg-secondary-fixed px-2.5 py-1 font-mono text-xs font-bold uppercase text-black">
            {tag}
          </span>
        ) : null}

        <div className="mt-3 flex flex-wrap items-center gap-4 text-xs text-[#686862]">
          <button
            ...
            onClick={toggleLike}
          >
            <CampusIcon name={liked ? "favorite" : "favorite_border"} className=" text-[17px]" />
            {formatMetric(likes)}
          </button>
          <button className="inline-flex items-center gap-1.5 hover:text-[#171717]" type="button" onClick={sharePost}>
            <CampusIcon name="arrow_outward" className=" text-[17px]" />
            Share
          </button>
          {isAnnouncement ? (
            post.registrationLink ? (
              <a className="inline-flex items-center gap-1.5 rounded border border-[#deded8] px-2 py-1 hover:text-[#171717]" href={post.registrationLink}>
                <CampusIcon name="how_to_reg" className=" text-[17px]" />
                Apply
              </a>
            ) : (
              <span aria-disabled="true" className="inline-flex items-center gap-1.5 rounded border border-[#deded8] px-2 py-1 opacity-50">
                <CampusIcon name="how_to_reg" className=" text-[17px]" />
                Apply
              </span>
            )
...
function EventRow({ event }: { event: ClubEvent }) {
  ...
  return (
    <div className="grid grid-cols-[44px_1fr] items-center gap-3 rounded border border-primary/10 bg-primary-fixed/55 px-3 py-2.5">
      <div className="text-center font-mono">
        <div className="text-base font-bold leading-none">{date.day}</div>
        <div className="mt-1 text-xs tracking-[0.08em] text-[#72726c]">{date.month}</div>
      </div>
      <div className="min-w-0">
        <p className="truncate text-xs font-bold">{event.title}</p>
        <p className="mt-1 truncate text-xs text-[#72726c]">{event.location || "Location not available"}</p>
      </div>
    </div>
  );
}
...
function MemberRow({ member }: { member: ClubMember }) {
  ...
  return (
    <Link className="flex items-center gap-2.5 py-2 hover:text-[#171717]" href={memberProfileHref(member)}>
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded border border-primary/15 bg-primary-fixed text-xs font-bold text-black">
        {member.initials || getInitials(member.name)}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block truncate text-xs font-semibold">{member.name}</span>
        <span className="mt-0.5 block truncate text-xs text-[#72726c]">{joinedLabel(member.createdAt)}</span>
      </span>
      {isLeader ? (
        <span className="rounded border border-[#d7d7d1] px-1.5 py-0.5 font-mono text-[8px] uppercase text-[#70706a]">
...
export function ClubHub({ detail }: ClubHubProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="mt-4 flex flex-col items-start gap-5 rounded border border-primary/12 bg-white p-5  sm:p-6 md:flex-row">
        <div className="flex h-[72px] w-[72px] shrink-0 items-center justify-center overflow-hidden rounded border border-primary/15 bg-primary-fixed text-lg font-bold text-black">
          {club.bannerImage ? (
        ...
        <div className="min-w-0 flex-1">
          ...
          <dl className="mt-4 flex flex-wrap gap-x-7 gap-y-3">
            <div>
              ...
              <dt className="mt-0.5 text-xs text-[#72726c]">Followers</dt>
            </div>
            <div>
          ...
          <div className="mt-4 flex w-fit items-center gap-2 rounded border border-secondary/15 bg-secondary-fixed px-3.5 py-1.5 font-mono text-xs uppercase text-black">
            <span className={cn("h-[7px] w-[7px] rounded", recruiting ? "animate-pulse bg-secondary" : "bg-outline")} />
            {club.status || "Status —"}
          ...
          <div className="min-w-0">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-[#72726c]">Showing all {posts.length} of {postsCount}</p>
              </div>
              <ClubPostComposer
                clubSlug={club.slug}
                members={members}
                triggerClassName="h-9 rounded border border-primary bg-primary px-3 text-xs font-bold text-white  hover:bg-primary/90"
              />
            </div>

            {posts.length === 0 ? (
              <div className="rounded border border-[#deded8] bg-white px-6 py-12 text-center">
              ...
              <div className="flex flex-col gap-3">
                {posts.map((post) => (
                  <ClubHubPostCard key={post.postId ?? post.title} post={post} />
                ))}
              </div>
            )}
          </div>
          ...
          <aside className="flex flex-col gap-3">
            <section className="rounded border border-[#deded8] bg-white p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <button className="text-xs text-[#94948d]" disabled type="button">See all</button>
              </div>
              {events.length > 0 ? (
                ...
                <div className="rounded border border-primary/10 bg-primary-fixed/55 px-3 py-4 text-xs leading-5 text-black">
            ...
            <section className="rounded border border-[#deded8] bg-white p-4">
              <div className="mb-1 flex items-center justify-between">
                ...
                <button className="text-xs text-[#72726c] hover:text-[#171717]" type="button" onClick={() => setActiveTab("members")}>
                  See all
                </button>
              </div>
              {members.length > 0 ? (
                ...
                <p className="py-3 text-xs text-[#72726c]">No members yet.</p>
              )}
            </section>

            <section className="rounded border border-[#deded8] bg-white p-4">
              <h2 className="text-[13px] font-semibold">Activity</h2>
              <p className="mt-3 flex items-center gap-2 font-mono text-xs uppercase text-[#70706a]">
                <span className="h-[7px] w-[7px] rounded bg-secondary" />
                Latest post · {latestPostTime}
              </p>
              <p className="mt-2 text-xs leading-5 text-[#72726c]">
      ...
      {activeTab === "members" ? (
        <section aria-labelledby="club-tab-members" className="mt-5" id="club-panel-members" role="tabpanel">
          ...
          {members.length > 0 ? (
            <div className="grid gap-x-8 rounded border border-[#deded8] bg-white px-4 py-2 md:grid-cols-2">
              {members.map((member) => <MemberRow key={member.id} member={member} />)}
            </div>
          ) : (
            <div className="rounded border border-[#deded8] bg-white px-6 py-12 text-center text-sm text-[#72726c]">
              No members yet.
            </div>
          )}
        </section>
      ...
      {activeTab === "about" ? (
        <section aria-labelledby="club-tab-about" className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]" id="club-panel-about" role="tabpanel">
          <div className="rounded border border-[#deded8] bg-white p-5">
