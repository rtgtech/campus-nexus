...
export function ClubMembersButton({ members }: { members: ClubMember[] }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        render={
          <Button className="rounded bg-white/14 px-4 text-xs font-bold uppercase tracking-[0.18em] text-white backdrop-blur-sm hover:bg-white/24" />
        }
      >
        View members
      </DialogTrigger>
      <DialogContent className="max-h-[82vh] max-w-xl overflow-hidden rounded p-0">
        <DialogHeader className="border-b border-surface-container-highest p-5 pr-14">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-on-secondary-fixed-variant">Members</p>
